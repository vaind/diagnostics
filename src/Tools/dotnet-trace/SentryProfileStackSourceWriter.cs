...
namespace Microsoft.Diagnostics.Tools.Trace {
    public static class SentryProfileStackSourceWriter {
        /// <summary>
        ...
        private static void WriteToFile(IReadOnlyDictionary<string, IReadOnlyList<ProfileEvent>> sortedProfileEventsPerThread,
            ...
            bool isFirst = true;
            foreach (var perThread in sortedProfileEventsPerThread.OrderBy(pair => pair.Value[0].RelativeTime)) {
                if (!isFirst)
                    writer.Write(", ");
                else
                    isFirst = false;
                ...
                writer.Write($"\"startValue\": {sortedProfileEvents[0].RelativeTime.ToString("R", CultureInfo.InvariantCulture)}, ");
                writer.Write($"\"endValue\": {sortedProfileEvents[sortedProfileEvents.Count - 1].RelativeTime.ToString("R", CultureInfo.InvariantCulture)}, ");
                writer.Write("\"events\": [ ");
